...
import { Loading ,Grid,Spacer ,Textarea,Collapse,Input,Row,Button,Modal,Text,Card,Avatar} from "@nextui-org/react";
import {AiOutlineSearch,AiOutlineProject,AiFillStar,AiOutlineStar  } from "react-icons/ai"
import {MdOutlineDelete,MdOutlineSubtitles} from "react-icons/md"
import {BsCashCoin,BsPencil} from "react-icons/bs"
...
const Proposal = ()=>{
    ...
    useEffect(()=>{
        ...
        setId(id);
        axios.post("http://localhost:3001/getproposal",{
            data:id,
        }).then(res=>{
    ...
    return(
        <Layout select="#0070f3" fori="job">
            <LayoutSecond disabled="not_posts" >
                    <div className={styles.mainTwo}>
                    {islaoding && <Loading type="default" /> || (
                        proposal.length > 0 && (
                            <Myprop id={id} prop={proposal} />
                ...
                <Grid.Container gap={2}>
                    <Grid style={{ width: '100%' }}>
                        ...
                        <Collapse
                            ...
                            shadow
                            bordered
                            title={<h3 className="nextui-collapse-title">Total bids <span style={{ color: 'red' }}>{propinfo.length}</span></h3>}
